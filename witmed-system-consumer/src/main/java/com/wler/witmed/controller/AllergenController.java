...
@RequestMapping("/system/allergen")
public class AllergenController extends BaseController {
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Allergen query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Allergen entity) throws Exception {
		// 获得 Code
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Allergen entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Allergen allergen = transport.getByCode(code);
			if (allergen == null || allergen.getCode().equals(entity.getCode())) {
