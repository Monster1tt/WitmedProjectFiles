...
public class SurveyController extends BaseController {
	@Autowired
	private SurveyTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Survey query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Survey> pageVO = new PageVO<Survey>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Survey> queryVO = new QueryVO<Survey>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Survey query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Survey entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Survey survey = transport.getByCode(code);
			if (survey == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Survey entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Survey survey = transport.getByCode(code);
			if (survey == null || survey.getCode().equals(entity.getCode())) {
