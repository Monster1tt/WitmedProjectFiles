...
@RequestMapping("/system/allergy")
public class AllergyController extends BaseController {
	@Autowired
	private AllergyTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Allergy query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Allergy> pageVO = new PageVO<Allergy>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Allergy> queryVO = new QueryVO<Allergy>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Allergy query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Allergy entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Allergy allergy = transport.getByCode(code);
			if (allergy == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Allergy entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Allergy allergy = transport.getByCode(code);
			if (allergy == null || allergy.getCode().equals(entity.getCode())) {
