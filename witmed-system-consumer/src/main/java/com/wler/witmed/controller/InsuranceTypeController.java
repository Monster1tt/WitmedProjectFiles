...
public class InsuranceTypeController extends BaseController {
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody InsuranceType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<InsuranceType> pageVO = new PageVO<InsuranceType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<InsuranceType> queryVO = new QueryVO<InsuranceType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody InsuranceType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody InsuranceType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			InsuranceType insuranceType = transport.getByCode(code);
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody InsuranceType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			InsuranceType insuranceType = transport.getByCode(code);
			if (insuranceType == null || insuranceType.getCode().equals(entity.getCode())) {
