 ...
 * @version 1.0.0
 */
@RestController("illnessTypeController")
@RequestMapping("/system/illnessType")
public class IllnessTypeController extends BaseController {
	@Autowired
	private IllnessTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody IllnessType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<IllnessType> pageVO = new PageVO<IllnessType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<IllnessType> queryVO = new QueryVO<IllnessType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody IllnessType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody IllnessType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			IllnessType illnessType = transport.getByCode(code);
			if (illnessType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody IllnessType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			IllnessType illnessType = transport.getByCode(code);
			if (illnessType == null || illnessType.getCode().equals(entity.getCode())) {
