 ...
 * @version 1.0.0
 */
@RestController("booldTypeController")
@RequestMapping("/system/booldType")
public class BooldTypeController extends BaseController {
	@Autowired
	private BooldTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody BooldType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<BooldType> pageVO = new PageVO<BooldType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<BooldType> queryVO = new QueryVO<BooldType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody BooldType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody BooldType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			BooldType booldType = transport.getByCode(code);
			if (booldType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody BooldType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			BooldType booldType = transport.getByCode(code);
			if (booldType == null || booldType.getCode().equals(entity.getCode())) {
