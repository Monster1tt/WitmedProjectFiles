 ...
 * @version 1.0.0
 */
@RestController("allergyTypeController")
@RequestMapping("/system/allergyType")
public class AllergyTypeController extends BaseController {
	@Autowired
	private AllergyTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody AllergyType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<AllergyType> pageVO = new PageVO<AllergyType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<AllergyType> queryVO = new QueryVO<AllergyType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody AllergyType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody AllergyType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			AllergyType allergyType = transport.getByCode(code);
			if (allergyType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AllergyType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			AllergyType allergyType = transport.getByCode(code);
			if (allergyType == null || allergyType.getCode().equals(entity.getCode())) {
