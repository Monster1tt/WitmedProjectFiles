...
@RequestMapping("/system/addressType")
public class AddressTypeController extends BaseController {
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody AddressType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody AddressType entity) throws Exception {
		// 获得 Code
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AddressType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			AddressType addressType = transport.getByCode(code);
			if (addressType == null || addressType.getCode().equals(entity.getCode())) {
