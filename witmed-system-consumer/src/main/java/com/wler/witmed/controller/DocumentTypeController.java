 ...
 * @version 1.0.0
 */
@RestController("documentTypeController")
@RequestMapping("/system/documentType")
public class DocumentTypeController extends BaseController {
	@Autowired
	private DocumentTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody DocumentType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<DocumentType> pageVO = new PageVO<DocumentType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<DocumentType> queryVO = new QueryVO<DocumentType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody DocumentType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody DocumentType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			DocumentType documentType = transport.getByCode(code);
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DocumentType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			DocumentType documentType = transport.getByCode(code);
			if (documentType == null || documentType.getCode().equals(entity.getCode())) {
