...
import com.wler.witmed.transport.system.CostTransport;
 ...
 * @version 1.0.0
 */
@RestController("costController")
@RequestMapping("/system/cost")
public class CostController extends BaseController {
	@Autowired
	private CostTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Cost query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Cost> pageVO = new PageVO<Cost>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Cost> queryVO = new QueryVO<Cost>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Cost query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Cost entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Cost cost = transport.getByCode(code);
			if (cost == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Cost entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Cost cost = transport.getByCode(code);
			if (cost == null || cost.getCode().equals(entity.getCode())) {
