...
@RequestMapping("/system/gender")
public class GenderController extends BaseController {
	@Autowired
	private GenderTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Gender query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Gender> pageVO = new PageVO<Gender>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Gender> queryVO = new QueryVO<Gender>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Gender query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Gender entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Gender gender = transport.getByCode(code);
			if (gender == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Gender entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Gender gender = transport.getByCode(code);
			if (gender == null || gender.getCode().equals(entity.getCode())) {
