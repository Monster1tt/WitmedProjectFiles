...
@RequestMapping("/system/contactType")
public class ContactTypeController extends BaseController {
	@Autowired
	private ContactTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody ContactType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<ContactType> pageVO = new PageVO<ContactType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<ContactType> queryVO = new QueryVO<ContactType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody ContactType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody ContactType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			ContactType contactType = transport.getByCode(code);
			if (contactType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody ContactType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			ContactType contactType = transport.getByCode(code);
			if (contactType == null || contactType.getCode().equals(entity.getCode())) {
