 ...
 * @version 1.0.0
 */
@RestController("doctorTitleController")
@RequestMapping("/system/doctorTitle")
public class DoctorTitleController extends BaseController {
	@Autowired
	private DoctorTitleTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody DoctorTitle query) throws Exception {
		// 创建 PageVO 对象
		PageVO<DoctorTitle> pageVO = new PageVO<DoctorTitle>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<DoctorTitle> queryVO = new QueryVO<DoctorTitle>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody DoctorTitle query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody DoctorTitle entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			DoctorTitle doctorTitle = transport.getByCode(code);
			if (doctorTitle == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody DoctorTitle entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			DoctorTitle doctorTitle = transport.getByCode(code);
			if (doctorTitle == null || doctorTitle.getCode().equals(entity.getCode())) {
