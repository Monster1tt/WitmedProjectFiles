 ...
 * @version 1.0.0
 */
@RestController("symptomLevelController")
@RequestMapping("/system/symptomLevel")
public class SymptomLevelController extends BaseController {
	@Autowired
	private SymptomLevelTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody SymptomLevel query) throws Exception {
		// 创建 PageVO 对象
		PageVO<SymptomLevel> pageVO = new PageVO<SymptomLevel>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<SymptomLevel> queryVO = new QueryVO<SymptomLevel>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody SymptomLevel query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody SymptomLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			SymptomLevel symptomLevel = transport.getByCode(code);
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody SymptomLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			SymptomLevel symptomLevel = transport.getByCode(code);
			if (symptomLevel == null || symptomLevel.getCode().equals(entity.getCode())) {
