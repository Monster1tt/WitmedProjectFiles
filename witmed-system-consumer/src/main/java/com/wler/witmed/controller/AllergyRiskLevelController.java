 ...
 * @version 1.0.0
 */
@RestController("allergyRiskLevelController")
@RequestMapping("/system/allergyRiskLevel")
public class AllergyRiskLevelController extends BaseController {
	@Autowired
	private AllergyRiskLevelTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody AllergyRiskLevel query) throws Exception {
		// 创建 PageVO 对象
		PageVO<AllergyRiskLevel> pageVO = new PageVO<AllergyRiskLevel>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<AllergyRiskLevel> queryVO = new QueryVO<AllergyRiskLevel>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody AllergyRiskLevel query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody AllergyRiskLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			AllergyRiskLevel allergyRiskLevel = transport.getByCode(code);
			if (allergyRiskLevel == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AllergyRiskLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			AllergyRiskLevel allergyRiskLevel = transport.getByCode(code);
			if (allergyRiskLevel == null || allergyRiskLevel.getCode().equals(entity.getCode())) {
