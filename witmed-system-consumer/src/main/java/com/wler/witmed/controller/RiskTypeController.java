 ...
 * @version 1.0.0
 */
@RestController("riskTypeController")
@RequestMapping("/system/riskType")
public class RiskTypeController extends BaseController {
	@Autowired
	private RiskTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody RiskType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<RiskType> pageVO = new PageVO<RiskType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<RiskType> queryVO = new QueryVO<RiskType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody RiskType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody RiskType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			RiskType riskType = transport.getByCode(code);
			if (riskType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody RiskType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			RiskType riskType = transport.getByCode(code);
			if (riskType == null || riskType.getCode().equals(entity.getCode())) {
