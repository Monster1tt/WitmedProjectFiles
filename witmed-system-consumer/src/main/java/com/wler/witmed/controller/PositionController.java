...
import com.wler.witmed.transport.system.PositionTransport;
...
public class PositionController extends BaseController {
	@Autowired
	private PositionTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Position query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Position> pageVO = new PageVO<Position>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Position> queryVO = new QueryVO<Position>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Position query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Position entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Position position = transport.getByCode(code);
			if (position == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Position entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Position position = transport.getByCode(code);
			if (position == null || position.getCode().equals(entity.getCode())) {
