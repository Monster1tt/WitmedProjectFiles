...
/**
 * <b>系统功能-既往观察方法类型控制层类</b>
 *
 * @author Monster
 * @version 1.0.0
 */
@RestController("pastSurveyController")
@RequestMapping("/system/pastSurvey")
public class PastSurveyController extends BaseController {
	@Autowired
	private PastSurveyTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody PastSurvey query) throws Exception {
		// 创建 PageVO 对象
		PageVO<PastSurvey> pageVO = new PageVO<PastSurvey>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<PastSurvey> queryVO = new QueryVO<PastSurvey>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody PastSurvey query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody PastSurvey entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			PastSurvey pastSurvey = transport.getByCode(code);
			if (pastSurvey == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody PastSurvey entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			PastSurvey pastSurvey = transport.getByCode(code);
			if (pastSurvey == null || pastSurvey.getCode().equals(entity.getCode())) {
