 ...
 * @version 1.0.0
 */
@RestController("booldRhTypeController")
@RequestMapping("/system/booldRhType")
public class BooldRhTypeController extends BaseController {
	@Autowired
	private BooldRhTypeTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody BooldRhType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<BooldRhType> pageVO = new PageVO<BooldRhType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<BooldRhType> queryVO = new QueryVO<BooldRhType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody BooldRhType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody BooldRhType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			BooldRhType booldRhType = transport.getByCode(code);
			if (booldRhType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody BooldRhType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			BooldRhType booldRhType = transport.getByCode(code);
			if (booldRhType == null || booldRhType.getCode().equals(entity.getCode())) {
