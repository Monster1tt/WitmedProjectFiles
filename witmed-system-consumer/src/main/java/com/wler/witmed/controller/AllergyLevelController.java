 ...
 * @version 1.0.0
 */
@RestController("allergyLevelController")
@RequestMapping("/system/allergyLevel")
public class AllergyLevelController extends BaseController {
	@Autowired
	private AllergyLevelTransport transport;
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody AllergyLevel query) throws Exception {
		// 创建 PageVO 对象
		PageVO<AllergyLevel> pageVO = new PageVO<AllergyLevel>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<AllergyLevel> queryVO = new QueryVO<AllergyLevel>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody AllergyLevel query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody AllergyLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			AllergyLevel allergyLevel = transport.getByCode(code);
			if (allergyLevel == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody AllergyLevel entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			AllergyLevel allergyLevel = transport.getByCode(code);
			if (allergyLevel == null || allergyLevel.getCode().equals(entity.getCode())) {
