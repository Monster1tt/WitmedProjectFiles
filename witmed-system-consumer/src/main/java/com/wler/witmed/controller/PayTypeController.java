...
public class PayTypeController extends BaseController {
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody PayType query) throws Exception {
		// 创建 PageVO 对象
		PageVO<PayType> pageVO = new PageVO<PayType>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<PayType> queryVO = new QueryVO<PayType>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody PayType query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody PayType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			PayType payType= transport.getByCode(code);
			if (payType == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody PayType entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			PayType payType = transport.getByCode(code);
			if (payType == null || payType.getCode().equals(entity.getCode())) {
