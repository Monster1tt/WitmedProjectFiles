...
public class DepartmentController extends BaseController {
	...
	public ResponseVO getPageByQuery(@PathVariable("pageNum") Integer pageNum,
	                                 ...
	                                 @RequestBody Department query) throws Exception {
		// 创建 PageVO 对象
		PageVO<Department> pageVO = new PageVO<Department>(pageNum, pageSize);
		// 创建 QueryVO对象
		QueryVO<Department> queryVO = new QueryVO<Department>();
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody Department query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody Department entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 校验此时的 Code 是否重复
			Department department = transport.getByCode(code);
			if (department == null) {
				//保存对象
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody Department entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			Department department = transport.getByCode(code);
			if (department == null || department.getCode().equals(entity.getCode())) {
