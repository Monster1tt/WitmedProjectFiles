...
@RequestMapping("/system/maritalStatus")
public class MaritalStatusController extends BaseController {
	 ...
	 */
	@PostMapping("/list")
	public ResponseVO getListByQuery(@RequestBody MaritalStatus query) throws Exception {
	 ...
	 */
	@PostMapping("/save")
	public ResponseVO save(@RequestBody MaritalStatus entity) throws Exception {
		// 获得 Code
	 ...
	 */
	@PostMapping("/update")
	public ResponseVO update(@RequestBody MaritalStatus entity) throws Exception {
		// 获得 Code
		String code = entity.getCode();
		if (code != null && !"".equals(code)) {
			// 此时用户修改 Code ，判断 Code 是否重复
			MaritalStatus maritalStatus = transport.getByCode(code);
			if (maritalStatus == null || maritalStatus.getCode().equals(entity.getCode())) {
